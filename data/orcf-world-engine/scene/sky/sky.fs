...
void main(void) {
  ...
  gl_FragData[2].rgb = 3.0 * (300.0 * pow(gl_LightSource[0].ambient.rgb, vec3(4.0, 4.0, 4.0))) * pow(0.5, dist / 10000.0);
  gl_FragData[2].rgb *= (0.25 + 0.75 * pow((500 - Vertex.y) / 500.0, 2.0)) * 3.0;
  gl_FragData[2].rgb += gl_LightSource[0].diffuse.rgb * pow(0.5, dist / 2000.0) / 2.0;
  ...
  gl_FragData[2].rgb += gl_LightSource[0].diffuse.rgb * pow(len / 5500.0, 4.0) * clamp(5.0 - 5.0 * dot(normalize(pos), normalize(pos * vec3(1.0, 0.0, 1.0))), 0.0, 1.0);
  gl_FragData[2].a = 1.0;
}