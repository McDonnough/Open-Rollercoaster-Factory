...
var
  i, j, k: Integer;
begin
  VBO := TVBO.Create(Park.pTerrain.SizeX * Park.pTerrain.SizeY * 4, GL_T2F_N3F_V3F, GL_QUADS);
  j := 0;
  k := 0;
  for i := 0 to Park.pTerrain.SizeX * Park.pTerrain.SizeY - 1 do
    begin
    VBO.TexCoords[4 * i] := Vector(0, 0);
    VBO.Vertices[4 * i] := Vector(j, Park.pTerrain.HeightMap[j, k], k);
    VBO.Normals[4 * i] := Vector(0, 1, 0);

    VBO.TexCoords[4 * i + 1] := Vector(0, 0);
    VBO.Vertices[4 * i + 1] := Vector(j + 1, Park.pTerrain.HeightMap[j + 1, k], k);
    VBO.Normals[4 * i + 1] := Vector(0, 1, 0);

    VBO.TexCoords[4 * i + 2] := Vector(0, 0);
    VBO.Vertices[4 * i + 2] := Vector(j + 1, Park.pTerrain.HeightMap[j + 1, k + 1], k + 1);
    VBO.Normals[4 * i + 2] := Vector(0, 1, 0);

    VBO.TexCoords[4 * i + 3] := Vector(0, 0);
    VBO.Vertices[4 * i + 3] := Vector(j, Park.pTerrain.HeightMap[j, k + 1], k + 1);
