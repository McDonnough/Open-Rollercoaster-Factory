...
begin
  ...
  if ModuleManager.ModInputHandler.Key[K_UP] then
    begin
    if fSpeed < 0.03 then
      fSpeed := fSpeed + 0.0003 * FPSDisplay.MS
    end
  ...
  if ModuleManager.ModInputHandler.Key[K_A] then
    begin
    ActiveCamera.Position.X := ActiveCamera.Position.X + Sin(DegToRad(ActiveCamera.Rotation.Y - 90)) * FPSDisplay.MS * 0.03;
    ActiveCamera.Position.Z := ActiveCamera.Position.Z - Cos(DegToRad(ActiveCamera.Rotation.Y - 90)) * FPSDisplay.MS * 0.03;
    end;
