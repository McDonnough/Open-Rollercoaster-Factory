...
void main(void) {
  Vertex.xz = gl_Vertex.xy + Offset;
  float hm = clamp(Height - texture2D(HeightMap, Vertex.xz / TerrainSize).b * 256.0 - 0.1, 0.0, 1.0);
  ...
  float Displacement = (0.5 + 0.5 * bumpColor.a) * gl_Vertex.z * 0.4 * hm;
  Vertex.y = Height + Displacement;
