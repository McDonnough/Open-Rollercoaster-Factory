#version 120

#extension GL_EXT_gpu_shader4 : require

#define FxaaInt2 ivec2
#define FxaaFloat2 vec2
#define FxaaTexLod0(t, p) texture2DLod(t, p, 0.0)
#define FxaaTexOff(t, p, o, r) texture2DLodOffset(t, p, 0.0, o)

uniform sampler2D Texture;
uniform sampler2D HDRColor;
uniform ivec2 ScreenSize;
uniform float Gamma;

const int Samples = {{{owe.samples}}};

vec3 FxaaPixelShader(vec4 posPos, vec2 rcpFrame) {
/*---------------------------------------------------------*/
    #define FXAA_REDUCE_MIN   (1.0/128.0)
    #define FXAA_REDUCE_MUL   0.0
    #define FXAA_SPAN_MAX     8.0
/*---------------------------------------------------------*/
    vec3 rgbNW = FxaaTexLod0(Texture, posPos.zw).xyz;
    vec3 rgbNE = FxaaTexOff(Texture, posPos.zw, FxaaInt2(1,0), rcpFrame.xy).xyz;
    vec3 rgbSW = FxaaTexOff(Texture, posPos.zw, FxaaInt2(0,1), rcpFrame.xy).xyz;
    vec3 rgbSE = FxaaTexOff(Texture, posPos.zw, FxaaInt2(1,1), rcpFrame.xy).xyz;
    vec3 rgbM  = FxaaTexLod0(Texture, posPos.xy).xyz;
/*---------------------------------------------------------*/
    vec3 luma = vec3(0.299, 0.587, 0.114);
    float lumaNW = dot(rgbNW, luma);
    float lumaNE = dot(rgbNE, luma);
    float lumaSW = dot(rgbSW, luma);
    float lumaSE = dot(rgbSE, luma);
    float lumaM  = dot(rgbM,  luma);
/*---------------------------------------------------------*/
    float lumaMin = min(lumaM, min(min(lumaNW, lumaNE), min(lumaSW, lumaSE)));
    float lumaMax = max(lumaM, max(max(lumaNW, lumaNE), max(lumaSW, lumaSE)));
/*---------------------------------------------------------*/
    vec2 dir;
    dir.x = -((lumaNW + lumaNE) - (lumaSW + lumaSE));
    dir.y =  ((lumaNW + lumaSW) - (lumaNE + lumaSE));
/*---------------------------------------------------------*/
    float dirReduce = max((lumaNW + lumaNE + lumaSW + lumaSE) * (0.25 * FXAA_REDUCE_MUL), FXAA_REDUCE_MIN);
    float rcpDirMin = 1.0/(min(abs(dir.x), abs(dir.y)) + dirReduce);
    dir = min(FxaaFloat2(FXAA_SPAN_MAX, FXAA_SPAN_MAX), max(FxaaFloat2(-FXAA_SPAN_MAX, -FXAA_SPAN_MAX), dir * rcpDirMin)) * rcpFrame.xy;
/*--------------------------------------------------------*/
    vec3 rgbA = (1.0/2.0) * (
        FxaaTexLod0(Texture, posPos.xy + dir * (1.0/3.0 - 0.5)).xyz +
        FxaaTexLod0(Texture, posPos.xy + dir * (2.0/3.0 - 0.5)).xyz);
    vec3 rgbB = rgbA * (1.0/2.0) + (1.0/4.0) * (
        FxaaTexLod0(Texture, posPos.xy + dir * (0.0/3.0 - 0.5)).xyz +
        FxaaTexLod0(Texture, posPos.xy + dir * (3.0/3.0 - 0.5)).xyz);
    float lumaB = dot(rgbB, luma);
    if ((lumaB < lumaMin) || (lumaB > lumaMax))
      return rgbA;
    return rgbB;
}

void main(void) {
  vec3 HDRAverage = texelFetch2D(HDRColor, ivec2(0, 0), 0).rgb;
  float HDRLighting = length(HDRAverage);
  float HDRLightingFactor = 0.7 / mix(HDRLighting, 0.7, 0.25);

  // IF [ NEQ owe.samples 1 ]
  gl_FragColor = vec4(0.0, 0.0, 0.0, 1.0);
  for (int i = 0; i < Samples; i++) {
    for (int j = 0; j < Samples; j++)
      gl_FragColor.rgb += texelFetch2D(Texture, Samples * ivec2(floor(gl_FragCoord.xy)) + ivec2(i, j), 0).rgb;
  }
  gl_FragColor.rgb /= (Samples * Samples);
  // END
  // IF [ EQ owe.samples 1 ]
    // IF [ NEQ owe.s3d.mode 0 ]
//     gl_FragColor.rgb = texture2D(Texture, gl_TexCoord[0].xy).rgb;
    gl_FragColor.rgb = FxaaPixelShader(vec4(gl_TexCoord[0].xy, gl_TexCoord[0].xy - 0.5 / ScreenSize), 1.0 / ScreenSize);
    // END
    // IF [ EQ owe.s3d.mode 0 ]
    gl_FragColor.rgb = texture2D(Texture, gl_TexCoord[0].xy).rgb;
    // END
  gl_FragColor.a = 1.0;
  // END
  
  gl_FragColor *= gl_Color * HDRLightingFactor;
  gl_FragColor.rgb += 0.5 * max(0.0, max(gl_FragColor.r, max(gl_FragColor.g, gl_FragColor.b)) - 1.0);

  // IF [ EQ owe.gamma 1 ]
  gl_FragColor.rgb = vec3(pow(gl_FragColor.r, Gamma), pow(gl_FragColor.g, Gamma), pow(gl_FragColor.b, Gamma));
  // END
}