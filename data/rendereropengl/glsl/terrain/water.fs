...
void main(void) {
  ...
  vec3 normal = normalize((-1.0 + 2.0 * texture2D(BumpMap, Vertex.xz / 10.0 + gl_TexCoord[0].xy).rbg) - (-1.0 + 2.0 * texture2D(BumpMap, Vertex.xz / 5.0 + 0.5 * gl_TexCoord[0].yx).rbg) + vec3(0.0, 1.0, 0.0));
